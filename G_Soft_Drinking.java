...
Overall the friends have 4?*?5?=?20 milliliters of the drink, it is enough to make 20?/?3?=?6 toasts. The limes are enough for 10?*?8?=?80 
toasts and the salt is enough for 100?/?1?=?100 toasts. However, there are 3 friends in the group, so the answer is min(6,?80,?100)?/?3?=?2.

*/
import java.util.*;
import java.io.*;

public class G_Soft_Drinking  {
    ...
    public static void main(String[] args) {
        ...
        int drink = n[1] * n[2] / n[6];
        int limes = n[3]* n[4];
        int salt = n[5] / n[7];
        
        System.out.println(Math.min(Math.min(drink, limes), salt) / n[0]);
